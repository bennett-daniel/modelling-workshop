...
linear_discount_function <- function(amount, delay, discount_parameter){
...
all_discount_parameter <- rep(NA, times=n_participants)
...
for (p_ix in 1:n_participants){
  ...
  ## fit model
  model_fit <- optim(
    par     = c(runif(n=1, min=-1, max=0), runif(n=1, min=0,max=2)), # k, beta
...
all_results_linear <- list(
  all_discount_parameter = all_discount_parameter,
  all_beta               = all_beta,
...
save("all_results_linear", file=paste("results", "mle-results-linear.Rdata", sep=.Platform$file.sep))
