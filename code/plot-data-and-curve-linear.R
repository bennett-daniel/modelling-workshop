...
linear_discount_factor <- -72
delay = seq(from=0, to = 125, length.out = 100)
critical_amount = 20 - linear_discount_factor * (delay/365)
...
discount_function_frame <- data.frame(
  delay            = plot_delays,
  subjective_value = linear_discount_fun(amount = 20, delay = plot_delays/365, discount_parameter = linear_discount_factor) 
)
...
cowplot::save_plot(plot=joint_plot, filename=here::here("plots", sprintf("data_plot_linear_p%.0f_%0.2f.png", p_ix, linear_discount_factor)),base_height = 10, base_width=25, units="cm")
